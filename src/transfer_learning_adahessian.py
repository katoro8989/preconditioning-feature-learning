...
def train_with_p(init_model, config: Config, train_loader, test_loader, device):
    model = copy.deepcopy(init_model).to(device)
    loss_fn = nn.MSELoss()

    optim_adahessian = AdaHessian(model.parameters(), lr=config.lr, weight_decay=config.wd, hessian_power=-config.p)
    ...
    for ep in tqdm(range(config.epochs)):
        ...
        for xb, yb in train_loader:
            optim_adahessian.zero_grad()
            pred = model(xb)
            loss = loss_fn(pred, yb)
            loss.backward(create_graph=True)
            optim_adahessian.step()
...
def main():
    ...
    test_ds  = TensorDataset(X_test,  y_test_n)
    ...
    model, tr1, te1 = train_with_p(init_model, config, train_loader, test_loader, device)
    ...
    if config.out_path:
        save_dir = os.path.join(config.out_path, f"transfer_learning_adahessian_p_{config.p:.1f}")
...
if __name__ == "__main__":
    ...
    print(f"Final Train1 Loss = {tr1[-1]:.6f}, Final Test1 Loss = {te1[-1]:.6f}")
    print(f"Final Train2 Loss = {tr2:.6f}, Final Test2 Loss = {te2:.6f}")