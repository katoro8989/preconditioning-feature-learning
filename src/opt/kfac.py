...
class KFACOptimizer(optim.Optimizer):
    ...
    def _save_input(self, module, input):
        if torch.is_grad_enabled() and self.steps % self.TCov == 0:
            ...
            if self.steps == 0:
                ...
                print("cov matrix initialized", module)
    ...
    def _update_inv(self, m):
        ...
        # Check for NaN values and replace with identity if found
        if torch.isnan(self.m_aa[m]).any():
            print(f"Warning: NaN detected in m_aa[{m}], using identity matrix")
            self.m_aa[m] = torch.eye(self.m_aa[m].size(0), device=self.m_aa[m].device, dtype=self.m_aa[m].dtype)
            
        if torch.isnan(self.m_gg[m]).any():
            print(f"Warning: NaN detected in m_gg[{m}], using identity matrix")
            self.m_gg[m] = torch.eye(self.m_gg[m].size(0), device=self.m_gg[m].device, dtype=self.m_gg[m].dtype)
        
        # Add regularization to ensure positive definiteness
        damping = self.defaults['damping']
        reg_aa = damping * torch.eye(self.m_aa[m].size(0), device=self.m_aa[m].device, dtype=self.m_aa[m].dtype)
        reg_gg = damping * torch.eye(self.m_gg[m].size(0), device=self.m_gg[m].device, dtype=self.m_gg[m].dtype)
        
        self.m_aa[m] = self.m_aa[m] + reg_aa
        self.m_gg[m] = self.m_gg[m] + reg_gg
